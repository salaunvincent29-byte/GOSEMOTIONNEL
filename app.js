...
const HISTORY_KEY='gps-emotionnel-v6-history',SETTINGS_KEY='gps-emotionnel-v6-settings';
const safeStorage={get(key,fallback){try{const v=localStorage.getItem(key);return v===null?fallback:JSON.parse(v)}catch{return fallback}},set(key,value){try{localStorage.setItem(key,JSON.stringify(value));return true}catch{return false}},remove(key){try{localStorage.removeItem(key);return true}catch{return false}}};
...
const S={view:'home',mode:'short',questions:[],i:0,answers:{},unknown:0,result:null,stopped:false,mapKey:null,mapZone:'chest',mapMode:'body',saving:false};
const esc=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
const history=()=>safeStorage.get(HISTORY_KEY,[]);
function bindGlobalKeys(){document.onkeydown=e=>{if(S.view!=='question'||S.saving)return;if(e.key==='1')answer('yes');if(e.key==='2')answer('no');if(e.key==='3')answer('unsure');if(e.key==='Escape')setView('home')}}
function updateNetworkBadge(){let badge=document.getElementById('networkBadge');if(!badge){badge=document.createElement('div');badge.id='networkBadge';badge.className='network-badge';document.body.appendChild(badge)}badge.textContent=navigator.onLine?'Hors ligne disponible':'Mode hors connexion';badge.classList.toggle('offline',!navigator.onLine)}
window.addEventListener('online',updateNetworkBadge);window.addEventListener('offline',updateNetworkBadge);
function setView(v){S.view=v;document.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===v));render();requestAnimationFrame(()=>window.scrollTo({top:0,behavior:document.body.classList.contains('reduce-motion')?'auto':'smooth'}))}
function render(){try{const fn=({home,question,result,map,history:historyView,science,mapDetail})[S.view]||home;fn();bindGlobalKeys()}catch(error){console.error(error);app.innerHTML=`<section class="card"><p class="eyebrow">ERREUR</p><h2>Un problème d’affichage est survenu.</h2><p>Vos données enregistrées ne sont pas supprimées.</p><button class="primary" id="recoverBtn">Revenir à l’accueil</button></section>`;document.getElementById('recoverBtn')?.addEventListener('click',()=>{S.view='home';render()})}}
function home(){const hasDraft=S.questions.length>0&&S.i>0&&S.i<S.questions.length;const h=history();app.innerHTML=`<section class="card"><p class="eyebrow">VERSION 6 — FINALE</p><h2>Je décris mon corps. Le GPS propose des hypothèses.</h2><p>Une question à la fois, sans émotion à deviner.</p>${hasDraft?`<div class="notice safe"><strong>Parcours en cours :</strong> question ${S.i+1} sur ${S.questions.length}.<button class="secondary" id="resumeBtn">Reprendre</button></div>`:''}<div class="grid"><button class="choice" data-mode="short"><strong>Parcours court</strong><small>3 à 5 minutes. Signes principaux et contexte.</small></button><button class="choice" data-mode="full"><strong>Parcours complet</strong><small>7 à 10 minutes. Exploration corporelle plus fine.</small></button></div><div class="notice"><strong>Réponses possibles :</strong> Oui, Non ou Difficile à dire. L’incertitude est acceptée.</div><div class="home-actions"><button class="secondary" onclick="setView('map')">Explorer la carte corporelle</button><button class="secondary" onclick="setView('history')">${h.length?`Voir ${h.length} enregistrement${h.length>1?'s':''}`:'Ouvrir l’historique'}</button></div></section>`;document.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>start(b.dataset.mode));document.getElementById('resumeBtn')?.addEventListener('click',()=>{S.view='question';render()})}
function start(mode){
 if(S.questions.length&&S.i>0&&S.i<S.questions.length){if(!confirm('Commencer un nouveau parcours et abandonner le parcours en cours ?'))return}
...
function question(){
 ...
 <div class="answers"><button class="answer yes" data-a="yes"><span>Oui</span><small>Touche 1</small></button><button class="answer no" data-a="no"><span>Non</span><small>Touche 2</small></button><button class="answer unsure" data-a="unsure"><span>Difficile à dire</span><small>Touche 3</small></button></div><button class="text-button" id="quitQuestion">Quitter et reprendre plus tard</button>
 ...
 document.querySelectorAll('[data-a]').forEach(b=>b.onclick=()=>answer(b.dataset.a));document.getElementById('quitQuestion')?.addEventListener('click',()=>setView('home'));
}
function answer(a){
 if(S.saving)return;S.saving=true;
 const q=S.questions[S.i];S.answers[q.id]=a;if(a==='unsure')S.unknown++;
 if(q.safety && a==='yes'){S.stopped=true;S.saving=false;return safetyStop()}
 ...
 setTimeout(()=>{S.i++;S.saving=false;if(S.i>=S.questions.length){S.result=calculate();S.view='result'}render()},220);
}
function safetyStop(){
...
function calculate(){
 ...
 const den=Math.max(1,S.questions.filter(q=>!q.safety).length),answered=den-S.unknown,conf=Math.round(100*Math.max(0,answered)/den);
 return{mix:flo,confidence:conf};
}
function result(){
 const v=S.result.mix.filter(x=>x.pct>0).slice(0,6),m=v[0]&&v[0].raw>0?v[0]:null;
 ...
 <button id="save" class="primary">Enregistrer</button><button class="secondary" onclick="start('${S.mode}')">Recommencer</button><button class="text-button" onclick="setView('home')">Terminer sans enregistrer</button><div id="saveFeedback" aria-live="polite"></div></section>`;
 document.getElementById('save').onclick=save;
}
function save(){const h=history();const item={date:new Date().toISOString(),mode:S.mode,intensity:+document.getElementById('intensity').value,context:document.getElementById('context').value.trim(),note:document.getElementById('note').value.trim(),unknown:S.unknown,mix:S.result.mix.map(x=>({key:x.key,pct:x.pct})),answers:S.answers};h.unshift(item);const ok=safeStorage.set(HISTORY_KEY,h.slice(0,250));const feedback=document.getElementById('saveFeedback');if(!ok){if(feedback)feedback.innerHTML='<div class="notice danger">Impossible d’enregistrer sur cet appareil. Vous pouvez faire une capture d’écran.</div>';return}if(feedback)feedback.innerHTML='<div class="confirmation">✓ Panorama enregistré.</div>';setTimeout(()=>setView('history'),500)}
function map(){
 ...
 app.innerHTML=`<section class="card realistic-map-card">
   ...
   <div class="map-mode-tabs" role="tablist">
     <button class="map-tab ${S.mapMode==="body"?"active":""}" data-mode="body">Corps humain</button>
     <button class="map-tab ${S.mapMode==="compass"?"active":""}" data-mode="compass">Boussole émotionnelle</button>
   </div>

   <div id="bodyPanel" class="${S.mapMode==="body"?"":"hidden"}">
   ...
   <div id="compassPanel" class="${S.mapMode==="compass"?"":"hidden"}">
     ${emotionCompass()}
   </div>
 </section>`;
...
function historyView(){const h=history();app.innerHTML=`<section class="card"><p class="eyebrow">HISTORIQUE LOCAL</p><h2>Panoramas enregistrés</h2>${h.length?h.map((x,i)=>`<article class="history-item"><div class="row"><strong>${new Date(x.date).toLocaleString('fr-FR')}</strong><span>${x.intensity}/10</span></div><p>${x.mix.filter(y=>y.pct>0).slice(0,4).map(y=>`${STATES[y.key]?.name} ${y.pct}%`).join(' · ')}</p>${x.context?`<small>${esc(x.context)}</small>`:''}<button class="text-button delete-item" data-i="${i}">Supprimer cet enregistrement</button></article>`).join(''):'<div class="empty-state"><span>▤</span><h3>Aucun panorama enregistré</h3><p>Les résultats enregistrés apparaîtront ici.</p><button class="primary" onclick="setView(\'home\')">Commencer un parcours</button></div>'}${h.length?`<div class="history-actions"><button class="secondary" id="exportHistory">Exporter en JSON</button><button class="secondary danger-outline" id="clearHistory">Tout effacer</button></div>`:''}</section>`;document.querySelectorAll('.delete-item').forEach(b=>b.addEventListener('click',()=>{const list=history();list.splice(+b.dataset.i,1);safeStorage.set(HISTORY_KEY,list);render()}));document.getElementById('clearHistory')?.addEventListener('click',()=>{if(confirm('Effacer définitivement tout l’historique ?')){safeStorage.remove(HISTORY_KEY);render()}});document.getElementById('exportHistory')?.addEventListener('click',()=>{const blob=new Blob([JSON.stringify(h,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='gps-emotionnel-v6-historique.json';a.click();URL.revokeObjectURL(url)})}
function science(){
 app.innerHTML=`<section class="card"><p class="eyebrow">MÉTHODE ET LIMITES</p><h2>Ce que fait réellement la V6</h2>
 ...
 <div class="notice">La V6 est un outil d’auto-observation. Elle n’est ni un test diagnostique, ni un dispositif médical validé.</div></section>`;
}
function bindGlobalKeys(){document.onkeydown=e=>{if(S.view!=='question'||S.saving)return;if(e.key==='1')answer('yes');if(e.key==='2')answer('no');if(e.key==='3')answer('unsure');if(e.key==='Escape')setView('home')}}
function updateNetworkBadge(){let badge=document.getElementById('networkBadge');if(!badge){badge=document.createElement('div');badge.id='networkBadge';badge.className='network-badge';document.body.appendChild(badge)}badge.textContent=navigator.onLine?'Hors ligne disponible':'Mode hors connexion';badge.classList.toggle('offline',!navigator.onLine)}
window.addEventListener('online',updateNetworkBadge);window.addEventListener('offline',updateNetworkBadge);
...
document.getElementById('saveSettings').onclick=()=>{const o={font:font.value,contrast:contrast.checked,motion:motion.checked};safeStorage.set(SETTINGS_KEY,o);apply(o);dlg.close()};
function loadSettings(){const o=safeStorage.get(SETTINGS_KEY,{});font.value=o.font||'normal';contrast.checked=!!o.contrast;motion.checked=!!o.motion;apply(o)}
...
loadSettings();updateNetworkBadge();render();
